/* ui.c – Menu with universal input (keyboard + gamepad) */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
static const char *menu_items[] = { "Update Database", "Test Disc", "Load Disc", "Close" };
#define MENU_COUNT 4
...
static const uint8_t font[256][7] = {
    ...
    ['I'] = {0x0E,0x04,0x04,0x04,0x04,0x04,0x0E}, ['J'] = {0x01,0x01,0x01,0x01,0x01,0x11,0x0E},
    ...
    ['2'] = {0x0E,0x11,0x01,0x02,0x04,0x08,0x1F}, ['3'] = {0x1F,0x01,0x02,0x07,0x01,0x11,0x0E},
    ['4'] = {0x02,0x06,0x0A,0x12,0x1F,0x02,0x02}, ['5'] = {0x1F,0x10,0x1E,0x01,0x01,0x11,0x0E},
    ['6'] = {0x0E,0x10,0x10,0x1E,0x11,0x11,0x0E}, ['7'] = {0x1F,0x01,0x02,0x04,0x04,0x04,0x04},
...
static void draw_text_centered(int y, const char *txt, uint32_t color)
{
    ...
    int x = (fb_w - width) / 2;
    for (int i = 0; i < len; ++i) draw_char(x + i*6, y, txt[i], color);
}
...
/* Find any input device that supports keys */
static int find_any_input_device(void)
{
    ...
    while ((ent = readdir(d))) {
        ...
        /* Check if it supports keys */
        unsigned long evbit[2] = {0};
        if (ioctl(fd, EVIOCGBIT(0, EV_MAX), evbit) >= 0 && (evbit[0] & (1 << EV_KEY))) {
            has_keys = 1;
        }
...
int ui_init(void)
{
    ...
    input_fd = find_any_input_device();
    if (input_fd < 0) return fprintf(stderr, "no input device found\n"), -1;
    return 0;
}
...
void ui_show_menu(int sel)
{
    ...
    for (int i = 0; i < MENU_COUNT; ++i) {
        uint32_t col = (i == sel) ? 0x00FFFFFF : 0x00808080;
        draw_text_centered(y + i * 20, menu_items[i], col);
    }
}
...
int ui_wait_for_selection(void)
{
    ...
    while (1) {
        ...
        if (ev.type != EV_KEY || ev.value != 1) continue;  /* key down */

        /* Up / D-pad Up */
        if (ev.code == KEY_UP || ev.code == BTN_DPAD_UP) {
            if (sel > 0) { sel--; ui_show_menu(sel); }
        }
        /* Down / D-pad Down */
        else if (ev.code == KEY_DOWN || ev.code == BTN_DPAD_DOWN) {
            if (sel < MENU_COUNT-1) { sel++; ui_show_menu(sel); }
        }
        /* Enter or any button */
        else if (ev.code == KEY_ENTER || ev.code == BTN_A || ev.code == BTN_START || ev.code == BTN_SELECT) {
            return sel;
        }
        /* ESC or B button to cancel */
        else if (ev.code == KEY_ESC || ev.code == BTN_B) {
            return -1;
        }
    }
}
